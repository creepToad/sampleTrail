...
namespace cAlgo.Robots
{
    ...
    public class RobotForex : Robot
    {
        ...
        [Parameter("Max Volume", DefaultValue = 10000)]
        ...
        [Parameter("PipStep Exponent", DefaultValue = 1.1)]
        public double PipStepExponent { get; set; }

        [Parameter("Max PipStep", DefaultValue = 500)]
        public int MaxPipStep { get; set; }

        [Parameter("Trigger Position", DefaultValue = 30)]
        public int TriggerPosition { get; set; }
        
        [Parameter("Volume Multiplier", DefaultValue = 5)]
       ...
       private void ControlSeries() 
        {
                int BarCount = 25; // Define BarCount
                int Del = MaxOrders - 1; // Define Del
            ...
            if(PipStep == 0) 
                ...
                _pipstep = (int)(PipStep * Math.Pow(PipStepExponent, Account.Positions.Count));
                
                // Check if _pipstep exceeds MaxPipStep
                if (_pipstep > MaxPipStep)
                    _pipstep = MaxPipStep;
            ...
            switch(GetPositionsSide())
            {
                case 0:
                    if(Symbol.Ask < FindLastPrice(TradeType.Buy) - _pipstep * Symbol.PointSize)
                    {
                        ...
                        Print("Current Positions: " + Account.Positions.Count);
                        Print("Calculated NewVolume: " + NewVolume);
                        if(Account.Positions.Count == TriggerPosition - 1) 
                        {
                            NewVolume = MaxVolume * VolumeMultiplier;
                            Print("Trigger Position reached, updated NewVolume: " + NewVolume);
                        }
                        if(NewVolume > MaxVolume && Account.Positions.Count != TriggerPosition - 1) NewVolume = MaxVolume;
                        if(!(NewVolume < LotStep)) Trade.CreateBuyMarketOrder(Symbol, NewVolume);
                    }
                    break;
                case 1:
                    if(Symbol.Bid > FindLastPrice(TradeType.Sell) + _pipstep * Symbol.PointSize)
                    {
                        ...
                        if(NewVolume > MaxVolume && Account.Positions.Count != TriggerPosition - 1) NewVolume = MaxVolume;
                        if(!(NewVolume < LotStep)) Trade.CreateSellMarketOrder(Symbol, NewVolume);
                    }
                    break;
                }
        }
